...
## Training and test
### Training
To train BIRNet: All the results were evaluated on a system with a NVIDIA GeForce RTX 4090 GPU with 24 GB of memory, running on Ubuntu20.01 (64-bit).

Firstly, modify the --data-path parameter in run_train_BIRNet.sh to the root path of ImageNet1k
```
--data-path /path/to/imagenet/
```
Then run the follow scripts to train BIRNet.
```
bash run_train_BIRNet.sh
```
The trained models will be saved in the OUTPUT folder, OUTPUT is defined in run_train_BIRNet.sh

### Testing
Modify the --data-path parameter in run_test_BIRNet.sh to the root path of ImageNet1k
```
--data-path /path/to/imagenet/
```
Then test BIRNet on ImageNet on a single gpu run:
```
bash run_test_BIRNet.sh
```
...
```
bash run_BIRNet_CUB200.sh
bash run_BIRNet_Food101.sh
bash run_BIRNet_oxfordFlowers.sh
bash run_BIRNet_StanfordDogs.sh
```
...
|--------|------------|--------|-----------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| BIRNet   | 52.68 M    | 11.0 G | 82.3 %    | [Google Drive](https://drive.google.com/file/d/1DcYmyYM0enILJjaBT7gEMzPFvEftiQ83/view?usp=drive_link), [Baidu Netdisk](https://pan.baidu.com/s/14VYegT_o4kHkFyskvLhr0w?pwd=51t2) |
| BIRNet-B | 88.47 M    | 16.1 G | 84.0 %    | Release later                                                                                                                                                                   |
| BIRNet-L | 206.09 M   | 42.0 G | 84.9 %    | Release later                                                                                                                                                                   |

# BIRNet Test Results on fine-grained benchmarks
...
# BIRNet Test Results on applied medical datasets
